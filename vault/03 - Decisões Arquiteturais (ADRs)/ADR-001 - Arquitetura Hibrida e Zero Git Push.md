...
status: aprovado
data: 2026-09-12
tags: [adr, arquitetura-hibrida, offline-first, zero-git-push, seguranca]
---

# 📜 ADR-001: Arquitetura Híbrida Offline-First e Guardrail de Zero Git Push

> [!abstract] **Resumo da Decisão**
> O EditalAudit AI adota uma arquitetura híbrida estrita: toda a matemática orçamentária e cálculo de prazos roda no motor determinístico local (`LocalCrossEngine.js`), enquanto a IA generativa (Gemini) atua de forma desacoplada apenas para síntese semântica e pareceres M.U.S.A. Toda a persistência é 100% local com proibição mandatória de `git push`.
...
- Todas as alterações, relatórios de auditoria, logs e métricas são consolidados localmente nas pastas `vault/`, `docs/`, `tools/` e artefatos de sessão.
...
- [[03 - Decisões Arquiteturais (ADRs)/ADR-002 - Guardrail de Memoria Ativa no Vault|ADR-002: Guardrail de Memória Ativa no Vault]]
